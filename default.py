...
addon = xbmcaddon.Addon('plugin.video.PORNOFLIXXX')
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(PORNOFLIXXX,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(PORNOFLIXXX,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR yellow][B]- FAVORITES CHANNEL PORNOFLIXXX[/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
